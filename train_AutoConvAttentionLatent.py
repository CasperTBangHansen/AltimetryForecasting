...
from Models.AttentionConvLSTMAutoEncoderLatentSpace import SaveLoadModels
from Models.AttentionConvLSTMAutoEncoderLatentSpace.training_loop import train_validation_loop
from Models.AttentionConvLSTMAutoEncoderLatentSpace.Seq2SeqAttention import Seq2SeqAttention, InputModel, OutputModel
...
from Models.AttentionConvLSTMAutoEncoderLatentSpace.Loss import create_loss_function_diff
from Models.AttentionConvLSTMAutoEncoderLatentSpace.Dataloader import outlier_detect

AUTOENCODERFOLDER = Path("SavedModels", "autoencoder_checkpoints")
auto_encoder_name = "checkpoint_11000epochs_32channels_unmasked.pkl"
DATAPATH = Path(r"Data/Grids/south_africa_6thdeg_mss21.nc")
SAVEFOLDER = Path("SavedModels", "ConvLSTMAttentionAutoSouthAfrica6Deg128")
MODEL_NAME = "checkpoint_100.pkl"
SAVEFOLDER.mkdir(parents=True, exist_ok=True)
LOAD_MODEL = False
DEVICE = torch.device("cuda:0") if torch.cuda.is_available() else torch.device("cpu")
total_memory = torch.cuda.get_device_properties(DEVICE).total_memory
available_memory = total_memory - torch.cuda.max_memory_allocated()
print(f"Device = {DEVICE}, Total memory = {total_memory/1024**3:.2f}gb, available memory = {available_memory/ 1024**3:.2f}gb")


# Dataset parameters
default_batch_size = 20
latent_batch_size = 40
...
# Loss
criterion = create_loss_function_diff(nn.MSELoss)
...
def print_to_file_on_epoch(loss: Loss, _: _types.float_like, __: _types.float_like, train_mse_losses, train_grad_losses, val_mse_losses, val_grad_losses, n_epochs: int):
    ...
    logger.info(f"Device: {torch.cuda.max_memory_allocated()/1024**3:.2f}gb, Epoch: {loss.epoch:04d}/{n_epochs} Training Loss: {loss.training_loss:.7f} Validation Loss: {loss.validation_loss:.7f}")
...
def setup_data(
    ...
    # Dataloders
    train_loader = DataLoader(SLADataset(train_features, train_time, dataset_parameters, min_value = min_value, difference = difference), **kwargs_dataloader)
    validation_loader = DataLoader(SLADataset(validation_features, validation_time, dataset_parameters, min_value = min_value, difference = difference), **kwargs_dataloader)
...
if LOAD_MODEL:
   ...
   # Load autoencoder
encoder, decoder, _, _, _, fill_nan, _, train_start, train_end, validation_end, min_value, difference = load_autoencoder(
    AUTOENCODERFOLDER / auto_encoder_name,
    DEVICE
)
print(f"Loaded model and found min = {min_value} and difference = {difference}", flush=True)
...
latent_train_dataset = outlier_detect(encoder, decoder, train_loader, DEVICE, 0.6)
latent_validation_dataset = outlier_detect(encoder, decoder, validation_loader, DEVICE, 0.6, latent_train_dataset.mins, latent_train_dataset.differences)

latent_train_loader = DataLoader(latent_train_dataset, shuffle=False, batch_size = latent_batch_size)
latent_validation_loader = DataLoader(latent_validation_dataset, shuffle=False, batch_size = latent_batch_size)
...
for i in range(100):
    try:
        ...
        losses = train_validation_loop(
            ...
            val_loader = latent_validation_loader,
            criterion = criterion,
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=weight_decay)
        print(f"{datetime.now()} | Failed {i} times")