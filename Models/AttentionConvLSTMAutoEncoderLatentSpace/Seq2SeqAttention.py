...
class Seq2SeqAttention(nn.Module):
    ...
    def forward(
        ...
        for t in range(pred_seq_len):
            ...
            output = decoded_featurespace.unsqueeze(2)
            outputs[:, :, t] = output.squeeze(2)

            if random.random() < teacher_force_ratio:
                feature_space =  y[:, t].unsqueeze(1).unsqueeze(1)
            else:
                feature_space = output
    ...
    def get_latent_space(
        ...
        for t in range(pred_seq_len):
            ...
            output = decoded_featurespace.unsqueeze(1)
