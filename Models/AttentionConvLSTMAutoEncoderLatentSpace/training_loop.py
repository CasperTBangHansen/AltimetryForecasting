    ...
    with torch.no_grad():
        for features, target in loader:            
            ...
            if n_sequences is None:
                current_loss, _mse_losses, _grad_losses = criterion(output, target)
            else:
                current_loss, _mse_losses, _grad_losses = criterion(output[:, :n_sequences], target[:, :n_sequences])
...
def train(
    ...
    for i, (features, target) in enumerate(loader):       
        ...
        loss, _mse_losses, _grad_losses = criterion(output[:, :n_sequence], target[:, :n_sequence])
        mse_losses += _mse_losses
        grad_losses += _grad_losses
        loss.backward()
