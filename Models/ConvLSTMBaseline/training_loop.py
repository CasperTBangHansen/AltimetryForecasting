...
) -> Tuple[float, _types.float_like]:
    """Computes the criterion for the model and loader in evaluation mode"""
    val_loss: float = 0
    example_img = np.array(0, dtype=np.float32)
    model.eval()
    with torch.no_grad():
        for features, target, mask, _, _ in loader:            
            ...
            if n_sequences is None:
                current_loss = criterion(output, target, mask).item()
            else:
                current_loss = criterion(output[:, :n_sequences], target[:, :n_sequences], mask[:, :n_sequences]).item()
            val_loss += current_loss
            
            if example_img is None:
                example_img = output[0].cpu().numpy()
                m = mask[0, :n_sequences].cpu().numpy()
                example_img[m] = np.nan

    # Compute average loss
    return val_loss / len(loader), example_img
...
def train_validation_loop(
    ...
    device: torch.device,
    n_sequences: int,
    update_function: Callable[[Loss, _types.float_like, int], None] | None = None,
    path: Path | None = None,
    ...
    if not isinstance(teacher_forcing_ratios, Sequence):
    ...
    try:
        for epoch in range(start_epoch + 1, num_epochs + 1):
            ...
            val_loss, example_img = validate(model, val_loader, criterion, device, n_sequences)
            ...
            if update_function is not None:
                update_function(losses[-1], example_img, num_epochs)
