...
from ..AttentionConvLSTM import Attention, InputModel, OutputModel, Encoder, Decoder, Seq2SeqAttention
...
def load_checkpoint(path: Path, model_type: Type[Seq2SeqAttention], device: torch.device) -> Tuple[Seq2SeqAttention, torch.optim.Optimizer, Loss, DatasetParameters, str]:
    """Loads the model, optimizer and loss from the path"""
    checkpoint = torch.load(path, map_location=device)
    
    # Model
    checkpoint['device'] = device
    autoencoder_name = checkpoint.pop('autoencoder_name')
    ...
    return model, optimizer, loss, dataset_parameters, autoencoder_name